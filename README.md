...
**核心特性：每个网站独立调度、互不影响。** 添加网站时分别为它设定访问节奏（固定间隔 / 随机间隔 / 窗口内随机 N 次），各网站按自己的时间表运行。
...
- **部署频率**：monitor.py 只在有站点实际被访问时才写回数据，所以只有数据变化才触发 Vercel 重新部署，不会每 5 分钟无谓构建。
- **Actions 额度提示**：`*/5` 分钟检查频率约每月 8640 次运行。仓库保持 **Public** 时 Actions 免费无额度限制（推荐）；若转 **Private**，每月 2000 分钟免费额度会紧张（约 2500+ 分钟），需把 `monitor.yml` 的 cron 改回 `*/15 * * * *` 或 `*/30 * * * *` 以省额度（相应"窗口内随机 N 次"的完成粒度变粗）。
